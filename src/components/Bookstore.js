    ...
    return (
      <div className="Bookstore">
        <NavBar
          ...
          rightContent={[
            <Icon key="0" type="search" onClick={this.goToSearch}/>
          ]}
        >书城</NavBar>
        <div className="filter-wrap">
          ...
          <ul className="filter-item-wrap">
            {
              typeData.map(item => (
                <li key={item.id} onClick={this.getBookList.bind(this, item.id)} style={{ color: this.state.currentId === item.id ? '#e87646' : '#000' }}>{item.name}</li>
              ))
            }
          </ul>
        </div>
